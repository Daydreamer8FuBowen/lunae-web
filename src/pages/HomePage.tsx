...
import demo3Image from './assets/demo3-bg.png'
import './home.css'
...
export default function HomePage() {
  ...
  return <div className="index-page">
    <header className="index-header">
      ...
      <nav aria-label="主导航"><Link to={routePaths.tools}>工具箱 <ArrowUpRight size={13} /></Link><Link to={routePaths.showcase}>效果展示 <ArrowUpRight size={13} /></Link><Link to={routePaths.motionLabDemo}>动效实验 <ArrowUpRight size={13} /></Link><Link to={routePaths.demo3}>3D 创作 <ArrowUpRight size={13} /></Link></nav>
    </header>
    <main className="index-main">
      ...
      <section id="collection" className="index-collection" aria-label="项目入口">
        ...
        <Link to={routePaths.demo3} className="index-entry index-entry--demo3" onPointerMove={movePreview} onPointerLeave={resetPreview}>
          <div className="index-entry-visual index-demo3-visual" aria-hidden="true">
            <img src={demo3Image} alt="" />
            <span className="index-demo3-overlay"><strong>3D Creator</strong><small>MODEL · RENDER · MOTION</small></span>
            <span className="index-open-icon"><ArrowUpRight size={26} /></span>
          </div>
          <div className="index-entry-caption"><div><h2>3D 创作</h2><span>把想象，做成立体。</span></div><span className="index-count">PORTFOLIO<ArrowUpRight size={21} /></span></div>
        </Link>
      </section>
    </main>
